...
2. **技能与 Agent 对等** — 四个公开 Skill 共享；Actor 以 `assets/agents/*.toml` 为唯一合约真源；宿主侧 Agent 放在 `agents/{codex,claude,cursor}/`，文案可不同，硬字段必须一致。
3. **完整运行时对等** — Claude/Cursor 增加四命令 `commands/` 镜像，以及 session 边界注入与危险动作护栏 `hooks/`；hooks 只提示/拦截，不授予权限。
...
- 共享 `skills/*/SKILL.md` + `skills/*/agents/openai.yaml`（由 `sync-agents` 从 TOML 生成）。
...
- 三宿主均可发现并调用同一套四个公开命令意图：`loop-engineering`、`status`、`release`、`knowledge-evolution`。
- Actor 合约字段在三宿主与 TOML 之间机械一致；Reviewer / Release / `physical_action` 硬规则保持。
- Claude/Cursor 具备显式 slash commands 与护栏 hooks。
...
├── skills/                            # 共享四 Skill（SKILL.md 真源）
│   └── */agents/openai.yaml           # Codex 绑定（sync-agents 既有输出，语义不变）
...
├── commands/                          # Claude/Cursor 四命令镜像（Codex 不依赖）
├── hooks/
│   ├── claude/hooks.json + ...
│   ├── cursor/hooks.json + ...
│   └── scripts/                       # 可选共享护栏脚本
...
### 6.1 Commands（仅 Claude / Cursor）

根目录 `commands/` 提供与四 Skill 对齐的显式命令：
...
命令正文必须：先 `triggerctl classify`，遵守决策，再调用同一 `dist/cli/*`。  
Codex **不**依赖 `commands/`（继续 `$skill` + Skill 描述）。

### 6.2 Hooks（仅 Claude / Cursor）

| 意图 | Claude | Cursor |
|------|--------|--------|
| 会话边界注入（四命令、无物理权限、先 classify） | `SessionStart` | `sessionStart` |
...
- `.claude-plugin/plugin.json` / `.cursor-plugin/plugin.json`：声明 `skills`、`agents`、`commands`、`hooks` 路径（按宿主 schema 允许的字段）。
...
1. **Core** — 插件 id、四 Skill、tagline、Node/runtime、无 legacy；**Codex 清单仍必检**；版本 `0.3.5`。
2. **Host:Claude** — Claude 清单、十 agents、四 commands、hooks 可解析且脚本存在。
3. **Host:Cursor** — 同上。
...
- 明确：**Codex 安装与用法不变**；Claude/Cursor 为新增面。

## 8. 错误处理与失败语义

- 合约漂移、缺 agent、缺 command、hooks 坏路径 → validator / `sync-agents --check` **失败关闭**。
...
- Plugin validation：三清单版本、commands 覆盖、hooks 存在性、Codex 金丝雀。
- 手动烟测清单（实现后）：Codex 仍可用四 `$` 命令；Claude `/pi-loop-engineering:*`（或宿主实际命名空间）；Cursor 同等。
...
| 决策 | 选择 |
|------|------|
| 对等层级 | 安装发现 + Agent + 完整运行时（commands/hooks）同时满足 |
| Agent 布局 | 宿主分目录 `agents/{codex,claude,cursor}/` |
| 仓库布局 | 单根三清单 |
| Commands/Hooks | 命令镜像 + 护栏 hooks |
| 实现路线 | 合约真源 + 宿主适配（A） |
