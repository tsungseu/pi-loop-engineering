...
export interface CodeGraphHealthReport {
  ok: boolean;
...
function parseArguments(argv: readonly string[]): ParsedCommand {
  ...
  const spec = COMMAND_OPTIONS[command as keyof typeof COMMAND_OPTIONS]!;
  ...
  return { command: command as keyof typeof COMMAND_OPTIONS, options };
}
...
async function readRepositoryRules(workspace: string): Promise<string> {
...
function codeGraphIsMandatory(rules: string): boolean {
...
async function indexDirectoryPresent(workspace: string): Promise<boolean> {
...
async function inspectIndexHealth(workspace: string): Promise<IndexHealth> {
  ...
  if (await pathExists(markerPath)) {
    try {
      ...
      if (marker.healthy === false) reasons.push("marker reports unhealthy");
      if (marker.initialized === false) reasons.push("marker reports uninitialized");
      const pending = marker.pendingChanges;
      if (pending !== undefined) {
        const total = (pending.added ?? 0) + (pending.modified ?? 0) + (pending.removed ?? 0);
        if (total > 0) reasons.push("pending source changes");
      }
      if (marker.worktreeMismatch != null) reasons.push("worktree mismatch");
      if (marker.index?.reindexRecommended === true) reasons.push("full reindex required");
  ...
  try {
    ...
    const reasons: string[] = [];
    if (payload.initialized === false) reasons.push("not initialized");
    const pending = payload.pendingChanges;
    if (pending !== undefined) {
      const total = (pending.added ?? 0) + (pending.modified ?? 0) + (pending.removed ?? 0);
      if (total > 0) reasons.push("pending source changes");
    }
    if (payload.worktreeMismatch != null) reasons.push("worktree mismatch");
...
async function cliExploreAvailable(workspace: string): Promise<boolean> {
  const result = await runCodegraph(workspace, ["explore", "--help"], 5_000);
  return result.exitCode === 0;
}

function nativeResolution(degraded: boolean, reason?: string): CodeGraphResolution {
  if (reason === undefined) {
...
function blockedResolution(reason: string): CodeGraphResolution {
  return { mode: "BLOCKED", degraded: false, initialization_attempted: false, reason };
}

export async function resolveCodeGraph(request: CodeGraphRequest): Promise<CodeGraphResolution> {
...
export async function reportCodeGraphHealth(workspaceInput: string): Promise<CodeGraphHealthReport> {
...
export async function syncExistingCodeGraph(workspaceInput: string): Promise<CodeGraphResolution> {
  const workspace = await resolveWorkspace(workspaceInput);
  const rules = await readRepositoryRules(workspace);
  const mandatory = codeGraphIsMandatory(rules);
  const present = await indexDirectoryPresent(workspace);
...
async function run(parsed: ParsedCommand): Promise<unknown> {
  ...
  switch (command) {
    case "resolve": {
      ...
      const request: CodeGraphRequest = { workspace };
      if (mcpAvailable !== undefined) request.mcpAvailable = mcpAvailable;
