...
function parentGuardPath(target: string): string {
...
async function withParentGuard<T>(target: string, action: () => Promise<T>): Promise<T> {
...
function parseOwner(value: unknown): LockOwner | null {
  ...
  if (Date.parse(value.expiresAt) <= Date.parse(value.acquiredAt)) return null;
...
async function readOwner(target: string): Promise<OwnerRead> {
  try {
    const value: unknown = JSON.parse(await readFile(ownerPath(target), "utf8"));
    const owner = parseOwner(value);
    return owner === null ? { kind: "MALFORMED" } : { kind: "VALID", owner };
  } catch (error) {
    if (error instanceof SyntaxError || errorCode(error) === "ENOENT") return { kind: "MALFORMED" };
    throw error;
  }
}
...
function lockStateError(target: string, ownerRead: OwnerRead, now: number): LoopError {
...
async function allocateFencingToken(target: string): Promise<number> {
  ...
  let current = 0;
  try {
    const value: unknown = JSON.parse(await readFile(counterPath, "utf8"));
...
export async function reconcileLock(options: ReconcileLockOptions): Promise<LockReconciliation> {
  ...
  return withParentGuard(options.target, async () => {
    ...
    const current: OwnerRead = lockIsDirectory ? await readOwner(options.target) : { kind: "MALFORMED" };
