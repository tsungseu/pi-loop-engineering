...
const PROTECTED_DIRECTORY_ROOTS = ["src", "schemas", "dist"];
...
function normalizeExclusions(exclusions) {
    ...
    for (const path of normalized) {
        ...
        if (CONTROL_EXCLUSIONS.includes(path))
            continue;
...
let emptyHooksDirectoryPromise;
async function emptyHooksDirectory() {
...
async function gitSnapshot(root) {
    ...
    if (indexPathOutput === "")
        throw schemaError("Git returned an empty index path.");
...
async function walkAbsoluteFiles(directory, output) {
    let entries;
    try {
...
async function assertCleanSubmodules(snapshot) {
...
function externalEntry(binding) {
    if (binding.readOnly !== true)
        throw schemaError("External artifact materialization must be read-only.");
...
function cachedFilesystemEntry(cache, root, path, tracked) {
    if (cache === undefined)
        return filesystemEntry(root, path, tracked);
...
async function artifactEntries(root, bindings, cache, index) {
    const entries = [];
...
async function buildGitManifest(kind, options, sealedSnapshot, filesystemCache) {
...
function isHostAbsoluteSourceMapPath(path) {
    return isAbsolute(path)
        || /^[A-Za-z]:[\\/]/u.test(path)
        || /^\\\\/u.test(path)
        || /^file:\/\//iu.test(path);
}
async function validateRuntimeSourceMap(root, path) {
...
async function buildArtifactManifest(root, bindings, cache, index) {
    return contentManifest("artifact", await artifactEntries(await realpath(resolve(root)), bindings, cache, index));
}
function assertIgnoredInputsBound(snapshot, exclusions, declaredArtifacts) {
    const declaredFiles = new Set(declaredArtifacts
        .filter((binding) => binding.kind === "file")
        .map((binding) => normalizeRelativePath(binding.path)));
    for (const path of snapshot.ignored) {
        if (isExcluded(path, exclusions) || declaredFiles.has(path))
            continue;
...
async function assertSealedSnapshotStable(initial, cache) {
...
async function captureWorkspaceEntries(snapshot, declaredArtifacts, exclusions) {
    const paths = new Set([...snapshot.index.keys(), ...snapshot.untracked].filter((path) => !isExcluded(path, exclusions)));
    for (const binding of declaredArtifacts) {
        if (binding.kind === "file")
            paths.add(normalizeRelativePath(binding.path));
    }
    const cache = new Map();
    for (const path of [...paths].sort(compareText)) {
...
export async function sealWaveInput(options) {
    ...
    const workspace = await buildGitManifest("workspace", { root: options.root, include: ["**/*"], exclusions, declaredArtifacts }, snapshot, filesystemCache);
    ...
    return validateSchema("wave-input", { ...content, digest: sha256Hex(canonicalJsonBytes(content)) });
}
...
function executeBoundedProcess(options) {
    return new Promise((resolvePromise, rejectPromise) => {
        ...
        const terminateOnce = () => {
            if (terminationPath !== "NATURAL_EXIT")
                return;
        ...
        child.on("error", (error) => {
            ...
            if (bytes.byteLength <= options.maxStderrBytes)
                stderr.push(bytes);
        });
...
async function resolvedExecutable(path, cwd) {
    if (!isAbsolute(path))
        throw schemaError("Evidence executables must be absolute paths.", { path });
    const canonical = await realpath(resolve(cwd, path));
    if (!(await stat(canonical)).isFile())
        throw schemaError("Evidence executable identity must resolve to a file.", { path: canonical });
    return { path: canonical, digest: await sha256File(canonical) };
}
async function captureExecutableVersion(executablePath, versionArgs, cwd, environment, timeoutMs) {
    ...
    if (version === "")
        throw schemaError("Evidence executable version probe returned no version metadata.");
    return version;
}
...
export function verifyEvidenceBinding(record, expected) {
    ...
    if (JSON.stringify(record.argv) !== JSON.stringify(expected.argv))
        mismatches.push("argv");
    if (record.executable_path !== expected.executablePath)
        mismatches.push("executable_path");
    if (record.executable_digest !== expected.executableDigest)
        mismatches.push("executable_digest");
    if (JSON.stringify(record.version_argv) !== JSON.stringify(expected.versionArgv))
        mismatches.push("version_argv");
    if (record.cwd !== expected.cwd)
        mismatches.push("cwd");
    if (record.timeout_ms !== expected.timeoutMs)
        mismatches.push("timeout_ms");
    if (record.stdout_limit_bytes !== expected.maxStdoutBytes)
        mismatches.push("stdout_limit_bytes");
    if (record.stderr_limit_bytes !== expected.maxStderrBytes)
        mismatches.push("stderr_limit_bytes");
    if (record.environment_digest !== expected.environmentDigest)
        mismatches.push("environment_digest");
    if (Buffer.compare(canonicalJsonBytes(record.tool_versions), canonicalJsonBytes(expected.toolVersions)) !== 0)
        mismatches.push("tool_versions");
    if (record.artifact_manifest_digest !== expected.artifactManifestDigest)
        mismatches.push("artifact_manifest_digest");
