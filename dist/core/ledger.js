...
import { appendJsonLine, assertEnglishMachineStrings, atomicWriteJson, canonicalJsonBytes, } from "./atomic-json.js";
...
function snapshotFromState(state, sequence, eventHash) {
    return validateSchema("loop", {
        ...state,
        last_event_sequence: sequence,
        last_event_hash: eventHash,
    });
}
function initialSnapshot(loopId) {
...
async function readSnapshot(path) {
    try {
        return validateSchema("loop", JSON.parse(await readFile(path, "utf8")));
    }
    catch (error) {
...
function createEvent(layout, sequence, previousHash, type, transactionId, dataDigest, timestamp) {
    ...
    return validateSchema("event", { ...withoutHash, hash });
}
async function readEvents(path) {
    ...
    for (const [index, line] of text.slice(0, -1).split("\n").entries()) {
        ...
        const event = validateSchema("event", value);
...
function parseTransactions(events) {
...
function assertArtifactLoop(artifact, loopId, code) {
...
function parseEnvelope(value, transaction, loopId) {
    if (!isRecord(value)) {
...
async function readEnvelope(layout, transaction) {
...
async function writeAll(handle, data) {
    ...
    while (offset < data.byteLength) {
        ...
        if (bytesWritten === 0)
            throw new Error("Ledger artifact write made no progress.");
        offset += bytesWritten;
    }
}
async function writePendingEnvelope(layout, transactionId, envelope, fault) {
...
function transitionError(message, snapshot, to, status) {
...
function nextTransitionState(snapshot, workflow, to, status) {
    ...
    if (to === "CANCELLED" || status === "CANCELLED") {
        ...
        if (to !== snapshot.phase)
            throw transitionError("NON_CONVERGENT must preserve the current phase.", snapshot, to, status);
    }
    else if (OVERLAY_STATUSES.has(status)) {
        if (to !== snapshot.phase)
            throw transitionError("Status overlays must preserve the current phase.", snapshot, to, status);
    }
...
async function quarantineFile(layout, path) {
...
class FileLoopLedger {
    layout;
    #workflow;
    #clock;
    #lockTtlMs;
    #fault;
    constructor(layout, workflow, options) {
        this.layout = layout;
    ...
    async #transact(kind, schema, expected, writeArtifact, nextState) {
        ...
        try {
            ...
            const intent = createEvent(this.layout, head.sequence + 1, head.hash, `${kind}_INTENT`, transactionId, intentDigest, timestamp);
            ...
            const committedArtifactPath = await lease.runExclusive(async () => (writePendingEnvelope(this.layout, transactionId, envelope, this.#fault)));
            const commit = createEvent(this.layout, intent.sequence + 1, intent.hash, `${kind}_COMMIT`, transactionId, envelopeDigest, this.#clock.now().toISOString());
    ...
    async #assertLifecyclePrerequisite(snapshot, to, status) {
    ...
    async transition(to, status, expected) {
        const snapshot = await this.snapshot();
        ...
        try {
            ...
            for (const event of events) {
                ...
                if (transaction === undefined)
                    throw new Error("Parsed transaction invariant failed.");
                ...
                if (envelope === undefined)
                    throw new Error("Committed transaction envelope invariant failed.");
            ...
            try {
                ...
                if (!(error instanceof LoopError) || error.code !== "RECONCILE_REQUIRED")
                    throw error;
            }
...
async function loadWorkflow() {
    ...
    for (const candidate of candidates) {
        try {
            return validateSchema("workflow-spec", JSON.parse(await readFile(candidate, "utf8")));
        }
        catch (error) {
            if (errorCode(error) !== "ENOENT")
                throw error;
...
export async function openLedger(layout, options = {}) {
    await mkdir(layout.loopRoot, { recursive: true });
    const ledger = new FileLoopLedger(layout, await loadWorkflow(), options);
    try {
        validateSchema("loop", JSON.parse(await readFile(layout.loopJson, "utf8")));
    }
    catch (error) {
        if (errorCode(error) !== "ENOENT")
            throw error;
