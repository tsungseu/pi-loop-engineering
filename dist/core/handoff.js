...
export async function writeCheckpoint(input) {
    ...
    const committed = await ledger.transact("CHECKPOINT", await ledger.cursor(), async () => {
        ...
        const checkpoint = validateSchema("checkpoint", {
            ...content,
...
export async function finalizeHandoff(input) {
    const layout = resolveLayout(input.workspace, input.loopId);
    const { snapshot, releaseRequiredGates } = await assertFinalizable(input, layout);
    const ledger = await openLedger(layout, input.fault === "after-handoff-commit"
        ? {
            fault: (point) => {
                if (point === "after-commit")
                    throw new Error("injected after-handoff-commit");
            },
        }
        : {});
    ...
    const committed = await ledger.transact("HANDOFF", await ledger.cursor(), async (transactionId) => {
        ...
        const handoff = validateSchema("handoff", {
            ...withSequence,
        ...
        if (input.fault === "after-pending-write")
            throw new Error("injected after-pending-write");
        await rename(pending, layout.handoffJson);
        if (input.fault === "after-pending-rename")
            throw new Error("injected after-pending-rename");
        return handoff;
    });
    if (input.fault === "before-phase-transition")
        throw new Error("injected before-phase-transition");
    await ledger.transition("HANDOFF_READY", "COMPLETE", await ledger.cursor());
    if (input.fault === "after-phase-transition")
        throw new Error("injected after-phase-transition");
    return committed.artifact;
}
export async function verifyHandoffFreshness(handoff, facts) {
    const mismatches = [];
    if (facts.sourceHeadSha !== handoff.source_head_sha)
        mismatches.push("source_head_sha");
    if (facts.reviewedTreeDigest !== handoff.reviewed_tree_digest)
        mismatches.push("reviewed_tree_digest");
    if (facts.workspaceDigest !== handoff.workspace_digest)
        mismatches.push("workspace_digest");
    if (facts.sourceManifestDigest !== handoff.source_manifest_digest)
        mismatches.push("source_manifest_digest");
    if (facts.runtimeManifestDigest !== handoff.runtime_manifest_digest)
        mismatches.push("runtime_manifest_digest");
    if (facts.projectPolicyDigest !== handoff.project_policy_digest)
        mismatches.push("project_policy_digest");
    if (facts.h1Digest !== handoff.h1_digest)
        mismatches.push("h1_digest");
    if (facts.loopMarkdownDigest !== handoff.loop_markdown_digest)
        mismatches.push("loop_markdown_digest");
    if (facts.evidenceManifestDigest !== handoff.evidence_manifest_digest)
        mismatches.push("evidence_manifest_digest");
...
export async function readHandoff(workspace, loopId) {
    const layout = resolveLayout(workspace, loopId);
    try {
        return validateSchema("handoff", JSON.parse(await readFile(layout.handoffJson, "utf8")));
    }
    catch (error) {
        if (errorCode(error) === "ENOENT")
            return null;
