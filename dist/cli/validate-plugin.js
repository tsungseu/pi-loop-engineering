 ...
 */
async function assertClaudeAgentsConvention(root, relativePath, manifest) {
    if ("agents" in manifest) {
        throw new ValidationError(".claude-plugin must not declare an agents field; Claude Code discovers agents/ by convention (anthropics/claude-code#21598).", { path: relativePath });
    }
...
async function assertHostPluginManifest(root, relativeDir, host) {
    ...
    if (host === "claude") {
        await assertClaudeAgentsConvention(root, relativePath, manifest);
    }
    else {
