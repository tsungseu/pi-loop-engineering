...
import { buildProposal, collectKnowledgeSources, defaultProposalReview, markProposalApplied, transitionProposal, } from "../core/knowledge.js";
...
function parseArguments(argv) {
    ...
    if (command === "propose") {
        if (options.workspace === undefined)
            throw new UsageError("Missing required option.", { option: "workspace" });
        if (loopIds.length === 0)
            throw new UsageError("Missing required option.", { option: "loop-id" });
        ...
        for (const key of ["workspace", "proposal-id", "to", "review"]) {
            if (options[key] === undefined)
        ...
        if (loopIds.length > 0)
            throw new UsageError("Unknown option.", { command, option: "loop-id" });
    }
    else {
        for (const key of ["workspace", "proposal-id", "implementation-loop-id"]) {
            if (options[key] === undefined)
                throw new UsageError("Missing required option.", { option: key });
        }
        for (const key of Object.keys(options)) {
...
function parseStatus(value) {
    if (!PROPOSAL_STATUSES.has(value)) {
...
//# sourceMappingURL=knowledgectl.js.map